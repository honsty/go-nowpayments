...
func TestMinimumAmount(t *testing.T) {
	...
	tests := []struct {
		...
		{"query parameters and route", args{"eur", "btc", "usd"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := MinimumAmount(tt.args.currencyFrom, tt.args.currencyTo, tt.args.fiatEquivalent)
			if tt.after != nil {
				tt.after(got, err)
			}
		})
